...
def _latest_compare_params_json() -> Path | None:
    compare_root = SCRIPT_DIR / RESULTS_ROOT_NAME / "compare_demo"
...
def _load_params(params_path: Path | None) -> tuple[dict, Path, str]:
    out_dir = ensure_dir(SCRIPT_DIR / RESULTS_ROOT_NAME / "publication_compare_latest" / "figures")
    if params_path is None:
        defaults = default_params()
        overrides = load_overrides_json(SCRIPT_DIR / "params_template_nonlinearPB.json")
        params = apply_param_overrides(defaults, overrides, reset_lambda_D=True)
        return params, out_dir, "params_template_nonlinearPB.json + default_params()"
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        "If omitted, the newest results_nonlinearPB/compare_demo/*/params.json is used.",
    )
    parser.add_argument(
        "--output-stem",
        type=str,
