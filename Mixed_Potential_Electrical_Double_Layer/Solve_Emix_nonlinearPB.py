...
Core assumptions in this nonlinear-PB variant:
- Nonlinear Poisson–Boltzmann bulk equation for a symmetric 1:1 electrolyte
- Linear charging boundary condition (same Stern/Robin form as the linear code)
...
RESULTS_ROOT_NAME = "results_nonlinearPB"
SOLVER_VARIANT_NAME = "nonlinearPB"
NATURE_COLORS = {
    "blue": "#355C7D",
    "orange": "#C06C52",
    "green": "#3B7A57",
...
def default_params() -> Dict[str, Any]:
    ...
    return dict(
        ...
        use_affine_phi2=False,             # affine phi2 shortcut is disabled in nonlinear PB
        use_closed_form_when_affine=False, # closed-form Emix shortcut is disabled in nonlinear PB
        ...
        dh_violation_action="ignore",      # nonlinear PB does not require the Debye-Huckel validity guard
        ...
        # nonlinear PB solver controls
        pb_nx=121,
        pb_ny=81,
        pb_ymax_tilde=12.0,
        pb_solver_tol=1e-8,
        pb_maxiter=250,
        pb_reuse_last_guess=True,
        ...
        heatmap_nx=25,
        heatmap_ny=25,
        scan_mode="FULL",  # nonlinear PB variant uses FULL by default
        heatmap_mode="FULL",
        sensitivity_mode="FULL",
        ofat_pH_min=0.0,
        ofat_pH_max=14.0,
...
def validate_params(params: Dict[str, Any]) -> None:
    ...
    if int(params.get("pb_nx", 1)) < 11 or int(params.get("pb_ny", 1)) < 11:
        raise ValueError("pb_nx and pb_ny must both be >= 11")
    if float(params.get("pb_ymax_tilde", 0.0)) <= 0:
        raise ValueError("pb_ymax_tilde must be positive")
    if float(params.get("pb_solver_tol", 0.0)) <= 0:
        raise ValueError("pb_solver_tol must be positive")
    if int(params.get("pb_maxiter", 0)) < 1:
        raise ValueError("pb_maxiter must be >= 1")
...
LinearDHEDLModel = EDLModel


class NonlinearPBModel:
    """
    Nonlinear Poisson-Boltzmann model on a 2D x-y domain.

    Governing PDE in dimensionless form for a symmetric 1:1 electrolyte:
        ∂²φ̃/∂x̃² + ∂²φ̃/∂ỹ² = sinh(φ̃)

    Boundary conditions:
        ∂φ̃/∂x̃ = 0 at x̃ = 0 and x̃ = L̃
        φ̃ = 0 at ỹ = y_max
        ∂φ̃/∂ỹ = -g_i (φ̃_M - φ̃ - φ̃_pzc,i) at ỹ = 0
        with piecewise-constant g_i and φ̃_pzc,i on Au/support/Pd.

    Notes:
    - The Robin/Stern charging boundary condition is kept identical to the
      linear code so the same kinetic coupling can be reused.
    - The affine decomposition and closed-form mixed-potential shortcut are not
      available in this nonlinear solver.
    """

    def __init__(self, params: Dict[str, Any]):
        self.params = copy.deepcopy(params)
        validate_params(self.params)
        self.derived: Dict[str, Any] = compute_derived_params(self.params)
        self.pre: Dict[str, Any] = {}
        self._last_E_mix: Optional[float] = None
        self._last_solution: Optional[np.ndarray] = None
        self._last_field_metrics: Dict[str, Any] = {}
        self._linear_seed_model: Optional[LinearDHEDLModel] = None
        self._build()
    ...
    def _build(self) -> None:
        ...
        pb_nx = int(p.get("pb_nx", 121))
        pb_ny = int(p.get("pb_ny", 81))
        y_max = float(p.get("pb_ymax_tilde", 12.0))

        L = float(d["L_tilde"])
        L_Au = float(d["L_Au_tilde"])
        L_C = float(d["L_C_tilde"])

        # Use a uniform x-grid for the nonlinear PB finite-difference solve.
        # Exact material boundaries are still inserted on the output/profile grid.
        x_solver = np.linspace(0.0, L, pb_nx)
        y_solver = np.linspace(0.0, y_max, pb_ny)
        x_profile = _surface_grid_with_boundaries(L, L_Au, L_C, int(p["Nx"]))

        dx = float(x_solver[1] - x_solver[0])
        dy = float(y_solver[1] - y_solver[0])

        g_surface = np.empty_like(x_solver)
        pzc_surface = np.empty_like(x_solver)
        tol = 1e-12
        mask_Au = x_solver <= L_Au + tol
        mask_Pd = x_solver >= L_C - tol
        mask_support = ~(mask_Au | mask_Pd)

        g_surface[mask_Au] = float(d["g_Au"])
        g_surface[mask_support] = float(d["g_C"])
        g_surface[mask_Pd] = float(d["g_Pd"])

        pzc_surface[mask_Au] = float(d["pzc_Au_tilde"])
        pzc_surface[mask_support] = float(d["pzc_C_tilde"])
        pzc_surface[mask_Pd] = float(d["pzc_Pd_tilde"])

        self.pre = dict(
            pb_nx=pb_nx,
            pb_ny=pb_ny,
            x_solver_tilde=x_solver,
            y_solver_tilde=y_solver,
            x_tilde=x_profile,
            dx=dx,
            dy=dy,
            y_max_tilde=y_max,
            g_surface=g_surface,
            pzc_surface=pzc_surface,
            a1=float("nan"),
            a2=float("nan"),
            b1=float("nan"),
            b2=float("nan"),
    ...
    def _linear_surface_seed(self, E_mix: float) -> np.ndarray:
        x_solver = self.pre["x_solver_tilde"]
        y_solver = self.pre["y_solver_tilde"]
        nx = int(self.pre["pb_nx"])
        ny = int(self.pre["pb_ny"])
        try:
            if self._linear_seed_model is None:
                self._linear_seed_model = LinearDHEDLModel(self.params)
            x_lin, phi_lin = self._linear_seed_model.phi_tilde_surface(E_mix)
            phi_surface = np.interp(x_solver, x_lin, phi_lin)
        except Exception:
            phi_surface = np.zeros(nx, dtype=float)

        decay = np.exp(-y_solver)[:, None]
        guess = decay * phi_surface[None, :]
        guess[-1, :] = 0.0
        return guess

    def _initial_guess(self, E_mix: float) -> np.ndarray:
        reuse_last = bool(self.params.get("pb_reuse_last_guess", True))
        if reuse_last and self._last_solution is not None and self._last_E_mix is not None:
            guess = np.array(self._last_solution, copy=True)
            delta = float(self.derived["beta"]) * (E_mix - self._last_E_mix)
            y_solver = self.pre["y_solver_tilde"]
            guess += np.exp(-y_solver)[:, None] * delta
            guess[-1, :] = 0.0
            return guess
        return self._linear_surface_seed(E_mix)

    def _residual_flat(self, phi_flat: np.ndarray, phiM_tilde: float) -> np.ndarray:
        nx = int(self.pre["pb_nx"])
        ny = int(self.pre["pb_ny"])
        dx = float(self.pre["dx"])
        dy = float(self.pre["dy"])
        g_surface = np.asarray(self.pre["g_surface"], dtype=float)
        pzc_surface = np.asarray(self.pre["pzc_surface"], dtype=float)

        phi = np.asarray(phi_flat, dtype=float).reshape((ny, nx))
        res = np.zeros_like(phi)

        # Bottom Robin boundary at y=0
        res[0, :] = (phi[1, :] - phi[0, :]) / dy + g_surface * (phiM_tilde - phi[0, :] - pzc_surface)

        # Interior nonlinear PB equation
        dx2 = dx * dx
        dy2 = dy * dy
        for j in range(1, ny - 1):
            d2x = np.empty(nx, dtype=float)
            d2x[0] = 2.0 * (phi[j, 1] - phi[j, 0]) / dx2
            d2x[1:-1] = (phi[j, 2:] - 2.0 * phi[j, 1:-1] + phi[j, :-2]) / dx2
            d2x[-1] = 2.0 * (phi[j, -2] - phi[j, -1]) / dx2
            d2y = (phi[j + 1, :] - 2.0 * phi[j, :] + phi[j - 1, :]) / dy2
            res[j, :] = d2x + d2y - np.sinh(np.clip(phi[j, :], -40.0, 40.0))

        # Top Dirichlet boundary
        res[-1, :] = phi[-1, :]
        return res.ravel()

    def _solve_field(self, E_mix: float) -> np.ndarray:
        phiM_tilde = float(self.derived["beta"]) * float(E_mix)
        phi = np.asarray(self._initial_guess(E_mix), dtype=float)
        tol = float(self.params.get("pb_solver_tol", 1e-8))
        maxiter = int(self.params.get("pb_maxiter", 250))
        nx = int(self.pre["pb_nx"])
        ny = int(self.pre["pb_ny"])
        dx = float(self.pre["dx"])
        dy = float(self.pre["dy"])
        ...
        inv_dx2 = 1.0 / (dx * dx)
        inv_dy2 = 1.0 / (dy * dy)
        base_diag = 2.0 * (inv_dx2 + inv_dy2)
        relax = 0.2

        phi = np.clip(phi, -15.0, 15.0)
        phi[-1, :] = 0.0
        ...
        for it in range(maxiter):
            ...
            # Bottom Robin boundary update
            phi0_target = (phi[1, :] / dy + g_surface * (phiM_tilde - pzc_surface)) / (1.0 / dy + g_surface)
            updated_bottom = (1.0 - relax) * phi[0, :] + relax * phi0_target
            max_update = max(max_update, float(np.max(np.abs(updated_bottom - phi[0, :]))))
            phi[0, :] = updated_bottom

            # Interior nonlinear PB sweeps with pointwise Newton updates
            for j in range(1, ny - 1):
                for i in range(nx):
                    west = phi[j, i - 1] if i > 0 else phi[j, 1]
                    east = phi[j, i + 1] if i < nx - 1 else phi[j, nx - 2]
                    south = phi[j - 1, i]
                    north = phi[j + 1, i]
                    A = inv_dx2 * (east + west) + inv_dy2 * (north + south)

                    u = float(phi[j, i])
                    for _ in range(3):
                        u_clip = float(np.clip(u, -20.0, 20.0))
                        fval = A - base_diag * u - math.sinh(u_clip)
                        dval = -base_diag - math.cosh(u_clip)
                        u -= fval / dval
                    u = float(np.clip(u, -20.0, 20.0))
                    new_val = (1.0 - relax) * float(phi[j, i]) + relax * u
                    max_update = max(max_update, abs(new_val - float(phi[j, i])))
                    phi[j, i] = new_val

                # Side Neumann boundary refresh during the sweep
                phi[j, 0] = phi[j, 1]
                phi[j, -1] = phi[j, -2]
            ...
            if it % 5 == 0 or it == maxiter - 1:
                last_residual = float(np.max(np.abs(self._residual_flat(phi.ravel(), phiM_tilde))))
                strict_update_tol = tol
                strict_residual_tol = max(10.0 * tol, 1e-6)
                soft_update_tol = max(10.0 * tol, 1e-3)
                soft_residual_tol = max(100.0 * tol, 5e-2)
                if max_update < strict_update_tol and last_residual < strict_residual_tol:
                    converged = True
                    break
                if max_update < soft_update_tol and last_residual < soft_residual_tol:
                    soft_converged = True
                    converged = True
                    break
        ...
        if not converged:
            raise RuntimeError(
                f"NonlinearPBModel failed to converge for E_mix={E_mix:.6g} V "
                f"(max_update={max_update:.3e}, residual={last_residual:.3e}, maxiter={maxiter})"
            )

        phi[-1, :] = 0.0
        self._last_solution = phi
        self._last_E_mix = float(E_mix)
        self._last_field_metrics = dict(
            field_iterations=it + 1,
            field_max_update=float(max_update),
            field_residual_max=float(last_residual),
            field_soft_converged=bool(soft_converged),
        )
        return phi

    def _surface_state(self, E_mix: float) -> Tuple[np.ndarray, np.ndarray]:
        phi = self._solve_field(E_mix)
        x_solver = np.asarray(self.pre["x_solver_tilde"], dtype=float)
        x_profile = np.asarray(self.pre["x_tilde"], dtype=float)
        phi_surface_solver = phi[0, :]
        phi_surface_profile = np.interp(x_profile, x_solver, phi_surface_solver)
        return x_profile, phi_surface_profile

    def phi_tilde_surface(self, E_mix: float) -> Tuple[np.ndarray, np.ndarray]:
        return self._surface_state(E_mix)

    def segment_mean_phi_tilde(self, E_mix: float) -> Tuple[float, float]:
        x, phi = self._surface_state(E_mix)
        L_Au = float(self.derived["L_Au_tilde"])
        L_C = float(self.derived["L_C_tilde"])
        L = float(self.derived["L_tilde"])
        mask_Au, mask_Pd = _segment_masks(x, L_Au, L_C, L)
        L_Pd = L - L_C
        Au_mean = trapz_compat(phi[mask_Au], x[mask_Au]) / max(L_Au, 1e-30)
        Pd_mean = trapz_compat(phi[mask_Pd], x[mask_Pd]) / max(L_Pd, 1e-30)
        return float(Au_mean), float(Pd_mean)

    def segment_mean_phi2(self, E_mix: float, use_affine_phi2: bool) -> Tuple[float, float]:
        del use_affine_phi2
        Au_t, Pd_t = self.segment_mean_phi_tilde(E_mix)
        scale = self.derived["R"] * self.derived["T"] / self.derived["F"]
        return float(scale * Au_t), float(scale * Pd_t)


def make_edl_model(params: Dict[str, Any]) -> NonlinearPBModel:
    return NonlinearPBModel(params)
...
def _build_run_output(
    ...
    return dict(
        solver_variant=SOLVER_VARIANT_NAME,
        mode=mode,
        E_mix=float(E_mix),
        i_mix=float(i_mix),
...
def _safe_scalar_eval(f: Callable[[float], float], x: float) -> Optional[float]:
    try:
        val = float(f(float(x)))
    except Exception:
        return None
...
def _find_safe_sign_change_bracket(
    f: Callable[[float], float],
    E_center: float,
    widths: Tuple[float, ...] = (0.02, 0.05, 0.1, 0.2, 0.3, 0.4),
    points_per_side: int = 4,
) -> Optional[Tuple[float, float]]:
    for halfspan in widths:
        pts = np.linspace(E_center - halfspan, E_center + halfspan, 2 * points_per_side + 1)
        vals = [_safe_scalar_eval(f, float(pt)) for pt in pts]
        for idx in range(len(pts) - 1):
            ...
            if v0 is None or v1 is None:
                continue
            if v0 == 0.0:
                return (float(pts[idx]) - 1e-6, float(pts[idx]) + 1e-6)
            if np.sign(v0) != np.sign(v1):
                return (float(pts[idx]), float(pts[idx + 1]))
    return None
...
def _solve_root_problem(
    ...
    fa = _safe_scalar_eval(f, a)
    fb = _safe_scalar_eval(f, b)
    expands = 0
    while (fa is None or fb is None or np.sign(fa) == np.sign(fb)) and expands < max_bracket_expands:
        mid = 0.5 * (a + b)
        span = b - a
        a = mid - 1.5 * span
        b = mid + 1.5 * span
    ...
    out_info.update(bracket_a=a, bracket_b=b, f_a=fa, f_b=fb, expands=expands)

    try:
        if fa is not None and fb is not None and np.sign(fa) != np.sign(fb):
            sol = root_scalar(f, bracket=(a, b), method="brentq", xtol=xtol)
            method = "brentq"
        else:
            x0 = float(E_guess)
            secant_pair: Optional[Tuple[float, float]] = None
            for delta in (0.02, -0.02, 0.05, -0.05, 0.1, -0.1):
                x1 = x0 + delta
                if _safe_scalar_eval(f, x0) is not None and _safe_scalar_eval(f, x1) is not None:
                    secant_pair = (x0, x1)
                    break
            if secant_pair is None:
                raise RuntimeError("No finite evaluation pair found for secant fallback")
            sol = root_scalar(f, x0=secant_pair[0], x1=secant_pair[1], method="secant", xtol=xtol, maxiter=200)
            method = "secant"
...
def full_mode_currents(E: float, edl: Any, params: Dict[str, Any], return_profiles: bool) -> Dict[str, Any]:
    """
    FULL mode:
...
def mean_mode_residual(E: float, edl: Any, params: Dict[str, Any], use_affine_phi2: bool) -> float:
...
def emix_closed_form_affine(edl: Any, params: Dict[str, Any]) -> float:
    """
...
def solve_emix(
    ...
    if isinstance(edl, NonlinearPBModel):
        if E_guess is None:
            E_guess = emix_closed_form_no_edl(edl.derived, params)
        if bracket is None:
            bracket = _find_safe_sign_change_bracket(f, float(E_guess))
        max_bracket_expands = 0 if bracket is not None else max_bracket_expands
...
def run_case(
    ...
    if use_edl:
        ...
        if mode == "MEAN" and use_affine_phi2 and use_closed and isinstance(edl, LinearDHEDLModel):
        ...
        out = _build_run_output(
            ...
            info=info,
            derived=edl.derived,
            a1=float(edl.pre.get("a1", float("nan"))),
            b1=float(edl.pre.get("b1", float("nan"))),
            a2=float(edl.pre.get("a2", float("nan"))),
            b2=float(edl.pre.get("b2", float("nan"))),
        )
        out.update(edl._last_field_metrics)
...
def plot_compare_polarization_curve(
    ...
    (I_edl_plot, I_no_plot), y_label, _ = _scaled_current_display(
        "I_net_abs",
        curve_edl["I_total_abs_A"],
        curve_no["I_total_abs_A"],
    )
...
def plot_compare_emix_imix(
    ...
    (i_mix_plot,), i_label, _ = _scaled_current_display("i_mix_abs", np.array([i_mix_abs_edl, i_mix_abs_no], dtype=float))
...
def _style_publication_axes(ax: Axes, xlabel: str, ylabel: str, title: str) -> None:
    ...
    ax.tick_params(labelsize=10.5, length=4.0, width=0.9, pad=2.5)
...
def plot_publication_compare_panels(
    ...
    The first three panels compare with/w/o EDL. The local current-density
    ...
    (i1_plot, i2_plot), i_label, _ = _scaled_current_display("local_current_density", i1, i2)
    i_mix_abs_vals = np.array(
        [
            np.nan if i_mix_abs_edl is None else float(i_mix_abs_edl),
            np.nan if i_mix_abs_no is None else float(i_mix_abs_no),
        ],
        dtype=float,
    )
    (i_mix_abs_plot,), i_mix_abs_label, _ = _scaled_current_display("i_mix_abs", i_mix_abs_vals)

    L_Au_nm = float(derived_edl["L_Au_tilde"]) * float(derived_edl["lambda_D"]) * 1e9
    L_C_nm = float(derived_edl["L_C_tilde"]) * float(derived_edl["lambda_D"]) * 1e9

    fig = plt.figure(figsize=(10.2, 6.5))
    gs = fig.add_gridspec(2, 2, width_ratios=[0.78, 1.22], wspace=0.34, hspace=0.34)
    gs_a = gs[0, 0].subgridspec(1, 3, width_ratios=[1.0, 0.28, 1.0], wspace=0.0)
    ...
    _style_publication_axes(ax_a1, "", r"Mixed potential, $E_{\mathrm{mix}}$ [V]", r"$E_{\mathrm{mix}}$")
    ax_a1.set_xticks(categories)
    ax_a1.set_xticklabels(labels)
    ...
    _style_publication_axes(ax_a2, "", i_mix_abs_label, r"$i_{\mathrm{mix}}$")
    ...
    ax_a2.tick_params(axis="y", labelleft=False, left=False, labelright=True, right=True)
    ax_a2.yaxis.labelpad = 1.0
    ax_a2.yaxis.set_label_coords(1.01, 0.5)
    ...
    fig.subplots_adjust(left=0.08, right=0.98, bottom=0.10, top=0.95, wspace=0.34, hspace=0.34)
...
def compare_edl_effects(
    ...
    save_dir: str | Path = f"{RESULTS_ROOT_NAME}/compare_case1",
...
def make_summary_row(run_tag: str, params: Dict[str, Any], result: Dict[str, Any], extra: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    row = dict(run=run_tag, solver_variant=SOLVER_VARIANT_NAME, **flatten_dict(params))
    row.update(
...
def make_ofat_specs(p0: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
    ...
    def cdl_scan_bounds(key: str) -> Tuple[float, float]:
        ...
        vmin = max(MIN_PHYSICAL_CDL_F_PER_M2, baseline / 10.0)
...
def run_ofat(base_params: Dict[str, Any], out_dir: Path, summary_rows: List[Dict[str, Any]]) -> None:
    ...
    for pname, spec in specs.items():
        ...
        if pname == "lambda_D" and (p0 is None):
            p0 = compute_derived_params(base_params)["lambda_D"]
...
def run_self_checks(
    ...
    _record(
        "top_boundary_zero",
        np.isfinite(top_max) and top_max < bc_tol,
    ...
    _record(
        ...
        dict(max_abs_phi_tilde=max_phi),
        warn_only=True,
    )
    ...
    if run_convergence:
        ...
        for pb_nx, pb_ny in [(81, 61), (121, 81), (161, 101)]:
            ...
            conv_rows.append(dict(pb_nx=pb_nx, pb_ny=pb_ny, E_mix=float(res["E_mix"]), i_mix=float(res["i_mix"])))
...
def run_full_workflow(params: Dict[str, Any], out_dir: str | Path, print_summary: bool = True) -> Dict[str, Any]:
    ...
    This is used by run_cases_nonlinearPB.py to batch-run multiple parameter sets.
    """
    p = copy.deepcopy(params)
...
def main() -> None:
    ...
    out_dir = SCRIPT_DIR / RESULTS_ROOT_NAME / timestamp
