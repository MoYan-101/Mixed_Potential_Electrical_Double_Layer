...
OUTPUT_TAG = f"same_length_i0_alpha050_au25_pd25_{BASE_RESULT_ID}"
...
OUT_BASE = ROOT / "Figures" / "Figure_same_length_i0_alpha"
...
def load_same_length_i0_alpha_params() -> dict[str, Any]:
    ...
    validate_same_length_i0_alpha_params(params)
    return params


def validate_same_length_i0_alpha_params(params: dict[str, Any]) -> None:
    checks = {
        "L_Au": L_AU_SAME,
        "L_Pd_len": L_PD_SAME,
        "L_gap": 10.0e-9,
...
def compute_same_length_i0_alpha_pair() -> dict[str, Any]:
    params = load_same_length_i0_alpha_params()
...
def load_summary_for_scripts() -> dict[str, str]:
    return summary_from_pair(compute_same_length_i0_alpha_pair())
...
def save_traceability_inputs(params: dict[str, Any] | None = None, summary: dict[str, str] | None = None) -> None:
    ensure_output_dirs()
    params = copy.deepcopy(params) if params is not None else load_same_length_i0_alpha_params()
...
def print_summary(summary: dict[str, str]) -> None:
    ...
    print(f"i_mix_abs_with = {float(summary['i_mix_abs_with']):.15g} A")
    print(f"i_mix_abs_no = {float(summary['i_mix_abs_no']):.15g} A")
